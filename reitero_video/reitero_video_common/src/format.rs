...
///
/// V2 format:
/// - Intra: timestamp(u64) + type(u8=1) + size(u32) + jpeg_rgb
...
pub enum PackedFrameData {
    /// JPEG-encoded RGB24 of the full frame.
    Intra { jpeg_rgb: Vec<u8> },
...
impl PackedFrame {
    pub fn new_intra(jpeg_rgb: Vec<u8>, timestamp_ms: u64) -> Self {
        Self {
            timestamp_ms,
            data: PackedFrameData::Intra { jpeg_rgb },
        }
    }
    ...
    pub fn payload(&self) -> &[u8] {
        match &self.data {
            PackedFrameData::Intra { jpeg_rgb } => jpeg_rgb,
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        match &self.data {
            PackedFrameData::Intra { jpeg_rgb } => {
                let size = jpeg_rgb.len() as u32;
                let mut bytes = Vec::with_capacity(8 + 1 + 4 + jpeg_rgb.len());
                bytes.extend_from_slice(&self.timestamp_ms.to_le_bytes());
                bytes.push(FrameType::Intra as u8);
                bytes.extend_from_slice(&size.to_le_bytes());
                bytes.extend_from_slice(jpeg_rgb);
                bytes
            }
            PackedFrameData::Inter {
                quality,
                ...
                bytes.push(global_mv.raw_flags());
    ...
    pub fn from_bytes(buf: &[u8]) -> Option<(Self, usize)> {
        ...
        match frame_type {
            FrameType::Intra => {
                ...
                let size = u32::from_le_bytes([buf[9], buf[10], buf[11], buf[12]]) as usize;
                let total = 8 + 1 + 4 + size;
                if buf.len() < total {
                    return None;
                }
                let jpeg_rgb = buf[13..13 + size].to_vec();
                Some((PackedFrame::new_intra(jpeg_rgb, timestamp_ms), total))
            }
            FrameType::Inter => {
                if buf.len() < 13 {
                    return None;
                ...
                let quality = buf[9];
                let global_mv = MotionVector::from_raw(buf[10] as i8, buf[11] as i8, buf[12]);
                if buf.len() < 17 {
                    return None;
                }
