...
struct InstrumentInner {
    // name -> (start time stack) to allow nested starts
    starts: HashMap<String, Vec<chrono::DateTime<Utc>>>,
    // name -> Measurement
...
impl Instrument {
    pub fn start_measure(name: &str) {
        ...
        INST.with(|i| {
            ...
            inner.starts.entry(key).or_default().push(Utc::now());
        });
    }
        ...
        INST.with(|i| {
            ...
            if let Some(mut stack) = inner.starts.remove(&key) {
                if let Some(start) = stack.pop() {
                    ...
                    let ns = elapsed.num_nanoseconds().unwrap_or(0).max(0) as u128;
