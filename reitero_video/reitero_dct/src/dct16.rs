...
/// 16×16 forward DCT transform matrix (rows = frequency basis vectors, columns = spatial
/// samples), split into two `i32x8` lanes so each row stores 16 coefficients in fixed‑point
/// form scaled by `1 << CONST_SCALE`.
...
/// 16×16 inverse DCT basis, matching `FDCT_TRANSPOSE_16` and using the same fixed‑point
/// scaling. Each row is an IDCT kernel applied to a single coefficient position.
...
/// 1‑D 16‑point forward DCT on a single row/column, using the fixed‑point basis above.
///
/// We treat the input as 16 scalar samples, multiply by the 16 pre‑scaled basis rows and
/// accumulate in i64 to keep plenty of headroom, then shift back down by `CONST_SCALE`.
fn fdct_1d_16(input: [i32x8; 2]) -> [i32x8; 2] {
    ...
    // Rounding bias before shifting back to the original scale.
    let round = 1i64 << (CONST_SCALE as i64 - 1);
    let mut out_low: [i32; 8] = [0; 8];
    let mut out_high: [i32; 8] = [0; 8];
    for j in 0..8 {
        // Defensive clamp; realistic DCT ranges are far inside i32.
...
/// 1‑D 16‑point inverse DCT on a single row/column.
///
/// This mirrors `fdct_1d_16`: we expand the two `i32x8` lanes into 16 scalars, multiply
/// each scalar by the corresponding IDCT basis row and accumulate, then apply the same
/// fixed‑point rounding/shift pattern.
...
/// Transpose a 16×16 matrix stored as `[[i32x8; 2]; 16]`.
///
/// We temporarily convert to a plain 16×16 integer array, transpose that, then pack back
/// into two‑lane `i32x8` rows; this avoids complicated SIMD shuffle code.
...
/// Encode an entire plane into 16×16 DCT blocks with per-block quantization steps.
///
/// Each block uses its own scalar quant step from `quant_steps[block_index]`.
/// Used for adaptive quantization.
pub fn encode_plane_16x16_aq(
    plane: &[i16],
    stride: usize,
    width: usize,
    height: usize,
...
/// Decode a full plane of 16×16 blocks with per-block quantization steps.
///
/// Each block uses its own scalar quant step from `quant_steps[block_index]`.
/// Used for adaptive quantization.
pub fn decode_plane_16x16_aq(
    coeffs: &[i16],
    ...
    for by in 0..blocks_y {
        for bx in 0..blocks_x {
            if skip_mask[mask_idx] {
                ...
                let q_fp: i32 = (qs * (1 << 14) as f32).round() as i32;
                let q_round = 1 << 13;
                for v in 0..16 {
                    let mut arr: [i32; 16] = [0; 16];
                    for u in 0..16 {
                ...
                for y in 0..16 {
                    ...
                    for x in 0..8 {
                        output_plane[offset + x] =
                            row_low[x].clamp(i16::MIN as i32, i16::MAX as i32) as i16;
                    }
                    for x in 0..8 {
                        output_plane[offset + x + 8] =
                            row_high[x].clamp(i16::MIN as i32, i16::MAX as i32) as i16;
                    }
                }
            }
            mask_idx += 1;
...
/// Encode an entire plane into 16×16 DCT blocks using a 16×16 quantization matrix.
///
/// Encode an entire plane into 16×16 DCT blocks using a 16×16 quantization matrix.
///
/// Each coefficient position (u,v) uses its own step from `quant_table[v*16 + u]`.
/// Skipped blocks appear as `None` entries in the returned vector.
pub fn encode_plane_16x16_matrix(
    plane: &[i16],
    stride: usize,
    width: usize,
    height: usize,
    ...
    let blocks_y = height / 16;
    let num_blocks = blocks_x * blocks_y;
    // Each coefficient position (u,v) uses its own quant step from `quant_table[v*16 + u]`.
    let skips = skip_mask
        .map(|m| m.to_vec())
        .unwrap_or(vec![false; num_blocks]);
    let mut result: Vec<Option<Vec<i16>>> = Vec::with_capacity(num_blocks);
        ...
        for bx in 0..blocks_x {
            if skips[mask_idx] {
                ...
                for v in 0..16 {
                    ...
                    let row_high = final_vecs[v][1].to_array();
                    for u in 0..8 {
                        let q_step = quant_table[v * 16 + u].max(1) as f32;
                        let q = (row_low[u] as f32 / q_step).round() as i16;
                        coeffs.push(q);
                    }
                    for u in 0..8 {
                        let q_step = quant_table[v * 16 + u + 8].max(1) as f32;
                        let q = (row_high[u] as f32 / q_step).round() as i16;
                        coeffs.push(q);
                    }
                }
                result.push(Some(coeffs));
...
/// Decode 16×16 DCT blocks back into a spatial plane, given a 16×16 quantization matrix.
///
/// Decode 16×16 DCT blocks back into a spatial plane using a 16×16 quantization matrix.
///
/// Uses `quant_table[v*16 + u]` as the dequant factor for each coefficient position.
pub fn decode_plane_16x16_matrix(
    coeffs: &[i16],
    output_plane: &mut [i16],
    stride: usize,
    width: usize,
    ...
    for by in 0..blocks_y {
        for bx in 0..blocks_x {
            if skip_mask[mask_idx] {
                ...
                let mut coeff_vecs: [[i32x8; 2]; 16] = [[i32x8::splat(0); 2]; 16];
                // Per‑coefficient dequantization: coeff * quant_table[v*16+u], clamped
                // to at least 1 to avoid a zero step.
                for v in 0..16 {
                    let mut arr: [i32; 16] = [0; 16];
                    for u in 0..16 {
                        let q = quant_table[v * 16 + u].max(1) as i32;
                        arr[u] = coeffs[coeff_idx] as i32 * q;
                        coeff_idx += 1;
                    }
                    coeff_vecs[v][0] = i32x8::new([
                        arr[0], arr[1], arr[2], arr[3], arr[4], arr[5], arr[6], arr[7],
