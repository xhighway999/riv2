use super::backend::{sad_block_halfpel_limit_luma, sad_block_int, sad_block_int_limit};
use super::{BLOCK_SIZE, LumaPlane, MotionVector, satd_scalar};
...
#[inline]
fn motion_vector_to_pixels(mv: MotionVector) -> (i32, i32) {
...
#[inline]
fn encode_halfpel_flag(frac_hp: i32) -> u8 {
    if frac_hp > 0 {
        0x01
    } else if frac_hp < 0 {
        0x02
...
fn hex_search_impl(
    ...
    #[cfg(feature = "threads")]
    {
        ...
        // Debug print (once) to verify threading configuration
        use std::sync::atomic::{AtomicBool, Ordering};
        static PRINTED: AtomicBool = AtomicBool::new(false);
        if !PRINTED.swap(true, Ordering::Relaxed) {
            println!("[Hex Search] Using {} threads", num_threads);
        }
...
fn process_block_hex(
    ...
    let zero_sad = if block_is_identical(curr_luma, prev_luma, width, height, x0, y0) {
        ...
        sad_block_int(prev_luma, curr_luma, width, height, x0, y0, 0, 0)
    };
    ...
    let dx_frac = dx_hp % 2;
    let dy_frac = dy_hp % 2;
    let frac_x_flag = encode_halfpel_flag(dx_frac);
    let frac_y_flag = encode_halfpel_flag(dy_frac);
    let flags = frac_x_flag | (frac_y_flag << 2);

    (
        MotionVector::from_raw(
...
pub fn rgb_to_luma_plane(rgb: &[u8], width: usize, height: usize) -> LumaPlane {
