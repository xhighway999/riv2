...
///
/// `data` must be `width * height * 3` bytes, row-major, no padding.
/// `timestamp` is in milliseconds and used for stream timing only.
pub struct Frame {
    pub data: Vec<u8>,
    pub width: u32,
    pub height: u32,
...
/// Stateful video encoder for the Reitero format.
///
/// Feed frames sequentially with [`Encoder::encode_frame`] and call
/// [`Encoder::finish`] to flush and retrieve the complete bitstream.
...
pub struct EncodeFrameStats {
    ...
    /// Size of residual payload for this frame (bytes)
...
impl<W: VideoWriter> Encoder<W> {
    ...
    fn encode_frame_data_with_stats(
        ...
        let mut frame_type = match self.config.keyframe_interval {
            ...
            KeyframeInterval::Automatic => {
                // Scene-change detection is not implemented; falls back to a fixed 30-frame interval.
        ...
        match frame_type {
            FrameType::Intra => {
                ...
                let stats = EncodeFrameStats {
                    ...
                    mv_bytes: 0,
                    mv_raw_bytes: 0,
                    residual_bytes: bytes.len().saturating_sub(8 + 1 + 1 + 4),
            ...
            FrameType::Inter => {
                ...
                for (i, mv) in mvs.iter().enumerate() {
                    ...
                    let mut flags = if mode == MvMode::New {
                        debug_assert!(
                            mv.subpixel_x() != -1 && mv.subpixel_y() != -1,
                            "Encoder canonicalization failed: -0.5 subpixel encountered",
                        );
                        mv.to_flags() & 0x0F // only fractional bits; skip set later
                    } else {
                ...
                let mut inter = ResidualEncoder::encode_inter(InterResidualEncodeParams {
                    ...
                    inter_dead_zone: self.config.inter_dead_zone,
                })
                .map_err(|e| {
                ...
                for idx in 0..mvs.len() {
                    ...
                    let mv_src = mvs[idx];
                    let skip = (block_flags[idx] & 0x40) != 0;
                    let (subpel_x, subpel_y) = if block_modes[idx] == MvMode::New {
                        let spx = match mv_src.subpixel_x() { 1 => reitero_residual::Subpel::PlusHalf, -1 => reitero_residual::Subpel::MinusHalf, _ => reitero_residual::Subpel::Zero };
                        let spy = match mv_src.subpixel_y() { 1 => reitero_residual::Subpel::PlusHalf, -1 => reitero_residual::Subpel::MinusHalf, _ => reitero_residual::Subpel::Zero };
                        (spx, spy)
                    } else {
                        (reitero_residual::Subpel::Zero, reitero_residual::Subpel::Zero)
                    };
                ...
                let mv_raw_bytes = mv_blocks.len() * 2 + new_blocks * 2; // mode+skip per block (1 bit), +deltas+subpels for NEW
                let mv_rans = self
                    .mv_rans_encoder
                ...
                // Must match what the decoder reconstructs — NOT the raw motion-search MVs.
                // Raw search MVs can diverge from coded_prefix when a predictor carries MinusHalf
                // subpixel (propagated from an earlier frame): the reconstructed MV retains the
                // predictor's component representation (e.g. dx=5, subpix=-1) while the raw search
                // always canonicalises to PlusHalf (e.g. dx=4, subpix=+1). Using raw MVs here
                // would cause Temporal-mode reconstruction to diverge from the decoder.
                self.prev_mvs = Some(coded_prefix.clone());
    ...
    /// Serialize footer to bytes. The current format has no footer.
