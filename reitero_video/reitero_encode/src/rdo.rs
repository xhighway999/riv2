...
impl LambdaEntry {
    pub fn for_quality(quality: u8, lambda_mult: f64) -> Self {
        ...
        let distortion_retention = 0.15 + (1.0 - normalized_q) * 0.55; // 0.15 .. 0.7
        let residual_rate_slope = 0.012 + normalized_q * 0.012; // 0.012 .. 0.024 bits per SATD unit
        let residual_rate_intercept = 12.0 + normalized_q * 8.0; // baseline bits per block
        let skip_flag_bits = 0.55 - normalized_q * 0.15; // encourage skip a bit more on low quality
