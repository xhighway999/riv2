...
use reitero_residual::decode_jpeg_rgb;
...
pub fn extract_frame_to_pwd(input: &Path, frame_index: u64) -> Result<PathBuf> {
    ...
    let expected_storage = header.frame_data_size();
    ...
    for i in 0..=frame_index {
        ...
        match pf.data {
            PackedFrameData::Intra { jpeg_rgb } => {
                let recon = decode_jpeg_rgb(&jpeg_rgb)?;
                if recon.len() != expected_storage {
                    anyhow::bail!("intra decoded size mismatch");
                }
                if i == frame_index {
                    ...
                    fs::write(out_dir.join("delta.jpg"), &jpeg_rgb)
                        .context("write delta.jpg (intra payload)")?;
                }
                let recon_yuv = Yuv420Frame::from_rgb(&recon, storage_w, storage_h)
                    .map_err(|e| anyhow::anyhow!("intra rgb->yuv failed: {e}"))?;
            ...
            PackedFrameData::Inter {
                ...
                for (block_idx, block) in mv_blocks.iter().enumerate() {
                    ...
                    mvs.push(MotionVector::from_raw(dx, dy, flags));
