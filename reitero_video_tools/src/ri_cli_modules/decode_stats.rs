...
pub struct DecodeStats {
    ...
    total_resid_rans_ns: u64,
    total_resid_deinterleave_ns: u64,
    total_resid_dct_y_ns: u64,
    total_resid_dct_uv_ns: u64,
    total_resid_apply_ns: u64,
    total_rgb_ns: u64,
}
...
impl DecodeStats {
    pub fn new(width: u32, height: u32, target_fps: u32) -> Self {
        ...
        Self {
            ...
            total_pred_ns: 0,
            total_resid_ns: 0,
            total_resid_rans_ns: 0,
            total_resid_deinterleave_ns: 0,
            total_resid_dct_y_ns: 0,
            total_resid_dct_uv_ns: 0,
            total_resid_apply_ns: 0,
            total_rgb_ns: 0,
        }
    }
    ...
    pub fn add_timings(&mut self, t: reitero_decode::DecodePhaseTimings) {
        ...
        // Add detailed residual counters
        self.total_resid_rans_ns += t.residual_rans_ns;
        self.total_resid_deinterleave_ns += t.residual_deinterleave_ns;
        self.total_resid_dct_y_ns += t.residual_dct_y_ns;
        self.total_resid_dct_uv_ns += t.residual_dct_uv_ns;
        self.total_resid_apply_ns += t.residual_apply_ns;
    ...
    pub fn print_summary(&self) {
        ...
        println!("    residual rans:       {:>10} ns ({:>5.1}%)", self.total_resid_rans_ns, pct(self.total_resid_rans_ns));
        println!("    residual deinterleave:{:>10} ns ({:>5.1}%)", self.total_resid_deinterleave_ns, pct(self.total_resid_deinterleave_ns));
        println!("    residual dct_y:      {:>10} ns ({:>5.1}%)", self.total_resid_dct_y_ns, pct(self.total_resid_dct_y_ns));
        println!("    residual dct_uv:     {:>10} ns ({:>5.1}%)", self.total_resid_dct_uv_ns, pct(self.total_resid_dct_uv_ns));
        println!("    residual apply:      {:>10} ns ({:>5.1}%)", self.total_resid_apply_ns, pct(self.total_resid_apply_ns));
