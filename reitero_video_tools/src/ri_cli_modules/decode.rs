...
pub enum DecodeOutputMode {
    ...
    Mpv,
    /// Write PPM frames to stdout (for piping into ffmpeg/analysis)
    Stdout,
}
...
impl FrameSink for StdoutSink {
    fn process_frame(&mut self, rgb_data: &[u8], _frame_idx: u64) -> Result<()> {
        // PPM header then raw RGB24 — ffmpeg reads via -f image2pipe -vcodec ppm
        write!(self.out, "P6\n{} {}\n255\n", self.width, self.height)?;
        self.out.write_all(rgb_data)?;
        Ok(())
    }
...
pub fn decode_video(
    ...
    match mode {
        DecodeOutputMode::File => eprintln!(
            "Decoding {} to {} as MP4",
            input,
            output.unwrap_or("output.mp4")
        ),
        DecodeOutputMode::Null => eprintln!("Decoding {} (null output)", input),
        DecodeOutputMode::Mpv => eprintln!("Decoding {} and piping to mpv", input),
        DecodeOutputMode::Stdout => eprintln!("Decoding {} to stdout as PPM sequence", input),
    }

    if skip_residuals {
        eprintln!("Note: Residual decoding is disabled - outputting motion-predicted frames only");
    }
    ...
    let mut sink: Box<dyn FrameSink> = match mode {
        ...
        DecodeOutputMode::Stdout => Box::new(StdoutSink::new(width, height)?),
    ...
    while decoder.has_more_frames() {
        match decoder.decode_frame() {
            Ok(frame) => {
                ...
                frame_idx += 1;
                if frame_idx % 30 == 0 {
                    eprint!("\rProcessed {} frames...", frame_idx);
                }
            }
            Err(e) => {
