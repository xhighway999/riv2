...
impl VideoInput {
    pub fn open(path: &str) -> Result<Self> {
        ...
        Self::from_format_context(format_context)
    }

    pub fn open_with_fps(path: &str, fps: &str) -> Result<Self> {
        let mut opts = ffmpeg::Dictionary::new();
        opts.set("framerate", fps);
        let format_context = ffmpeg::format::input_with_dictionary(&path, opts)
            .context("Failed to open input with framerate option")?;
        Self::from_format_context(format_context)
    }

    fn from_format_context(format_context: ffmpeg::format::context::Input) -> Result<Self> {
